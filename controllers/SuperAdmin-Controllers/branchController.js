...
const getAllBranches = async (req, res) => {
  try {
    ...
    const rawBranches = await branchModel
      .find(query)
      .select("branchName branchManagerName servicePinCode createdAt")
...
const getBranchById = async (req, res) => {
  try {
    ...
    const [deliveryBoys, products] = await Promise.all([
      DeliveryBoyModel.find({ branch: rawbranch._id  })
...
const availableProducts = async (req, res) => {
  try {
    ...
    // Filter out null or missing populated products
    ...
    // Count valid products in all records of this branch (not just paginated)
    const allBranchProducts = await BranchAdminProductModel.find({ branch: branchId })
      .select("product")
      .populate("product", "_id");

    const totalValidCount = allBranchProducts.filter(item => item.product).length;
    const totalPages = Math.ceil(totalValidCount / limit);
