...
const updateDeliveryBoy = async (req, res) => {
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(branch)) {
      // If branch is already an ObjectId (string), directly convert to ObjectId
      branchDoc = await BranchModel.findById(branch);
      if (!branchDoc) {
        return res.status(404).json({ success: false, message: "Branch not found." });
      }
    } else {
    ...
    deliveryBoy.branch = branchDoc._id; // Store the branch as ObjectId
