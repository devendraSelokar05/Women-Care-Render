...
const BuyOrderFromCart = async (req, res) => {
  try {
    ...
    if (!userPinCode) {
      return res
        .status(400)
        .json({ message: "No valid 6-digit PIN code found in your address." });
    }

    const matchedBranch = await branchModel.findOne({
      servicePinCode: userPinCode,
    });

    if (!matchedBranch) {
      return res
        .status(404)
        .json({ message: "No branch found matching your address's PIN code." });
    }
    ...
    if (!branchAdmin) {
      return res
        .status(404)
        .json({ message: "No branch admin found for this branch." });
    }
